...
export async function GET(
  ...
  try {
    ...
    doc
      ...
      .fillColor(NAVY)
      .text(
        invoice.isCreditNote ? "PURCHASE CREDIT NOTE" : "PURCHASE INVOICE",
        { align: "right" }
      )

    doc.moveDown(0.5)

    // --- Supplier details and invoice details side by side ---
    ...
    doc
      ...
      .fontSize(10)
      .fillColor(CORAL)
      .text("Supplier:", leftX, detailsY)

    doc.moveDown(0.3)
    doc.font("Helvetica").fontSize(10).fillColor(DARK_GRAY)

    doc.text(invoice.supplier.name, leftX)
    if (invoice.supplier.addressLine1)
      doc.text(invoice.supplier.addressLine1, leftX)
    if (invoice.supplier.addressLine2)
      doc.text(invoice.supplier.addressLine2, leftX)
    const cityLine = [
      invoice.supplier.city,
      invoice.supplier.county,
      invoice.supplier.postcode,
    ]
      .filter(Boolean)
      .join(", ")
    if (cityLine) doc.text(cityLine, leftX)
    if (invoice.supplier.country) doc.text(invoice.supplier.country, leftX)
    if (invoice.supplier.vatNumber) {
      ...
      doc
        ...
        .fontSize(8)
        .fillColor(MID_GRAY)
        .text(`VAT: ${invoice.supplier.vatNumber}`, leftX)
    }
    ...
    dy += 15
    if (invoice.supplier.accountCode) {
      detailLabel("Account Code:", invoice.supplier.accountCode, dy)
    }
    ...
    if (Number(invoice.paidAmount) > 0) {
      ...
      const outstanding = total - Number(invoice.paidAmount)
      addTotalLine(
        "Outstanding:",
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="PurchaseInvoice-${invoice.invoiceNumber}.pdf"`,
      },
    })
  } catch (error) {
    console.error("Purchase invoice PDF error:", error)
    return NextResponse.json(
