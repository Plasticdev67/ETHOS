...
export default function PurchaseInvoiceDetailPage() {
  ...
  useEffect(() => {
    async function fetchInvoice() {
      try {
        ...
        const res = await fetch(`/api/finance/purchase-invoices/${invoiceId}`)
  ...
  async function handlePost() {
    ...
    try {
      ...
      const res = await fetch(`/api/finance/purchase-invoices/${invoiceId}/post`, {
        method: 'POST',
      })
  ...
  async function handleCreateCreditNote() {
    try {
      ...
      const body = {
        reference: creditNoteReference.trim() || undefined,
      ...
      const res = await fetch(`/api/finance/purchase-invoices/${invoiceId}/credit-note`, {
        method: 'POST',
      ...
      router.push(`/finance/purchases/${creditNote.id}`)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred')
    } finally {
      setActionLoading(false)
  ...
  if (error && !invoice) {
    return (
      <div className="space-y-6">
        ...
        <Link href="/finance/purchases" className="btn-secondary inline-flex items-center gap-2">
          <ArrowLeft size={16} />
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              ...
              {invoice.isCreditNote && invoice.relatedInvoice && (
                <> &mdash; Credit for{' '}
                  <Link
                    href={`/finance/purchases/${invoice.relatedInvoice.id}`}
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            onClick={() => window.open('/api/finance/documents/purchase-order/' + invoiceId)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <div className="text-sm text-gray-600 space-y-1">
            ...
            {invoice.supplier?.contactName && <p>{invoice.supplier.contactName}</p>}
            {invoice.supplier?.email && <p>{invoice.supplier.email}</p>}
            {invoice.supplier?.addressLine1 && (
              <>
                <p>{invoice.supplier.addressLine1}</p>
                {invoice.supplier.addressLine2 && <p>{invoice.supplier.addressLine2}</p>}
                <p>
                  {[invoice.supplier.city, invoice.supplier.postcode].filter(Boolean).join(', ')}
                </p>
                {invoice.supplier.country && <p>{invoice.supplier.country}</p>}
              </>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="label">Credit Note Reference</label>
                <input
                  type="text"
                  className="input w-full"
                  value={creditNoteReference}
                  onChange={(e) => setCreditNoteReference(e.target.value)}
                  placeholder="Supplier's credit note reference (optional)"
                />
              </div>
