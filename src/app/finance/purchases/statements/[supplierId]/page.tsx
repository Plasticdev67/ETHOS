...
export default function SupplierStatementPage() {
  ...
  const [fromDate, setFromDate] = useState(formatDateISO(ninetyDaysAgo))
    ...
    async function fetchStatement() {
      try {
        ...
        const res = await fetch(
          `/api/finance/purchases/statements/${supplierId}?${queryParams.toString()}`
        )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 print:hidden">
        ...
        <span>/</span>
        <Link href={`/finance/purchases/suppliers/${supplierId}`} className="hover:text-gray-700">
          {data.supplier.name}
        </Link>
        <span>/</span>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 print:hidden">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{data.supplier.name}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <div className="text-sm text-gray-600 space-y-0.5">
            <p className="font-semibold text-gray-900">{data.supplier.name}</p>
            <p className="text-gray-400 font-mono text-xs">{data.supplier.code}</p>
            {data.supplier.contactName && <p>{data.supplier.contactName}</p>}
            {data.supplier.addressLine1 && (
              <>
                <p>{data.supplier.addressLine1}</p>
                {data.supplier.addressLine2 && <p>{data.supplier.addressLine2}</p>}
                <p>
                  {[data.supplier.city, data.supplier.county, data.supplier.postcode]
                    .filter(Boolean)
                    .join(', ')}
                </p>
                {data.supplier.country && <p>{data.supplier.country}</p>}
              </>
            )}
            {data.supplier.email && <p className="mt-2">{data.supplier.email}</p>}
