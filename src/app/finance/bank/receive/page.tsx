...
export default function ReceivePaymentPage() {
  ...
  const [receiptDate, setReceiptDate] = useState(formatDateISO(new Date()))
  ...
  // Select customer and fetch their outstanding invoices
  async function selectCustomer(cust: Customer) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/finance/sales/invoices?customerId=${cust.id}&status=POSTED&status=PARTIALLY_PAID&isCreditNote=false`
      )
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch('/api/finance/bank/receipts', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'Failed to record receipt')
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-900">Receive Payment</span>
      </div>
      ...
      {/* Section 2: Customer Selection & Invoice Allocation */}
      <div className="card p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">
        ...
        {selectedCustomer && !loadingInvoices && allocations.length === 0 && (
                ...
                <tbody className="divide-y divide-gray-200">
                  {allocations.map((alloc, index) => (
                    <tr
                      ...
                      className={cn(
                        'hover:bg-gray-50',
                        alloc.allocateAmount > 0 && 'bg-green-50'
                      )}
                    >
      ...
      {totalAmount > 0 && selectedCustomer && (
        <div className="card p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            Journal Preview
          </h2>
      ...
      <div className="flex justify-end gap-3 pb-8">
        ...
        <button
          ...
          disabled={saving || success || !totalAmount || !selectedCustomer}
