...
interface AgedDebtorsData {
  ...
  customers: AgedDebtorCustomer[]
}

export default function AgedDebtorsPage() {
  const [data, setData] = useState<AgedDebtorsData | null>(null)
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const res = await fetch(`/api/finance/sales-ledger/aged?asAtDate=${asAtDate}`)
        if (!res.ok) throw new Error('Failed to load aged debtors report')
        ...
        if (json.customers) {
          setExpandedCustomers(new Set(json.customers.map((c: AgedDebtorCustomer) => c.customerId)))
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-900">Aged Debtors Report</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Outstanding receivables broken down by age band
          </p>
        </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-sm font-medium text-gray-900 mb-1">No outstanding debtors</h3>
            <p className="text-sm text-gray-500">All invoices are paid up to {formatDate(asAtDate)}.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead>
                <tr>
                  <th className="table-header">Customer / Invoice</th>
                      ...
                      >
                        <td className="table-cell font-semibold text-gray-900">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-xs text-gray-400">{cust.customerCode}</span>
                          </div>
                        </td>
                      ...
                      {isExpanded && (
                        <tr className="bg-gray-50 border-t border-gray-300">
                          <td className="table-cell pl-10 text-xs text-gray-500 font-medium" colSpan={4}>
                            Subtotals: Current: {formatCurrency(cust.current)} | 30 Days: {formatCurrency(cust.days30)} | 60 Days: {formatCurrency(cust.days60)} | 90+ Days: {formatCurrency(cust.days90Plus)}
                          </td>
